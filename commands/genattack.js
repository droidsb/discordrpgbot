...
module.exports.run = async (client, message, args, prefix,ticks,users,items,locations,mobs,moblist,cooldown,server) => {
  ...
  if(searchid(message.author.id,searchid(usrserver,users).data).rank==="Admin"){
          ...
          message.content=message.content.replace(prefix+"genattack ", "")
    
    var dmg=message.content.split(" ")[0]
    ...
    var description=message.content.replace(dmg+" ","").replace("undefined","")
    
   
    
    message.channel.send("addattack {\"dmg\":"+Number(dmg)+",\"description\":\""+description+"\"}")
